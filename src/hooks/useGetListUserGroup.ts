...
import { IRequestGetListUserGroup, IResponseListUserGroup, IResponsePermission } from "../interface/usergroup";
...
const useGetListUserGroup = (
    ...
    const save = useSave();
    const [data, setData] = useState<IResponsePermission>();
        ...
        return new Promise((resolve, reject) => {
            (async () => {
                try {
                    ...
                    const response = await UserGroupService.getListUserGroup(nextFilters);
                    resolve(response);
                } catch (error) {
                    setError(error);
                    reject(error);
...
export default useGetListUserGroup;
